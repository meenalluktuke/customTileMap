define(function (require) {
  let module = require('plugins/customTileMap/ui/modules').get('kibana/config', [
    'kibana/notify'
  ]);

  require('plugins/customTileMap/ui/routes').addSetupWork(function (config) {
    return config.init();
  });
  ...
  module.service('config', function (Private, Notifier, kbnVersion, kbnIndex, $rootScope, buildNum) {
    ...
    let defaults = Private(require('plugins/customTileMap/ui/config/defaults'));
    let DelayedUpdater = Private(require('plugins/customTileMap/ui/config/_delayed_updater'));
    let vals = Private(require('plugins/customTileMap/ui/config/_vals'));

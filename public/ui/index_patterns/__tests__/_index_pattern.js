describe('index pattern', function () {
  ...
  beforeEach(ngMock.inject(function (Private, $injector, _config_) {
    ...
    DocSource = Private(require('plugins/customTileMap/ui/courier/data_source/doc_source'));
    sinon.stub(DocSource.prototype, 'doIndex');
    sinon.stub(DocSource.prototype, 'fetch');

    // stub mapper
    mapper = Private(require('plugins/customTileMap/ui/index_patterns/_mapper'));
    sinon.stub(mapper, 'getFieldsForIndexPattern', function () {
      return Promise.resolve(_.filter(mockLogstashFields, { scripted: false }));
    });

    // stub mappingSetup
    mappingSetup = Private(require('plugins/customTileMap/ui/utils/mapping_setup'));
    ...
    Private.stub(require('plugins/customTileMap/ui/index_patterns/_calculate_indices'), calculateIndices);

    // spy on intervals
    intervals = Private(require('plugins/customTileMap/ui/index_patterns/_intervals'));
    sinon.stub(intervals, 'toIndexList').returns([
      { index: 'foo', max: Infinity, min: -Infinity },
      { index: 'bar', max: Infinity, min: -Infinity }
    ]);

    IndexPattern = Private(require('plugins/customTileMap/ui/index_patterns/_index_pattern'));
  }));

define(function (require) {
  return function IndexPatternFactory(Private, timefilter, Notifier, config, kbnIndex, Promise, $rootScope, safeConfirm) {
    ...
    let getIds = Private(require('plugins/customTileMap/ui/index_patterns/_get_ids'));
    let mapper = Private(require('plugins/customTileMap/ui/index_patterns/_mapper'));
    let intervals = Private(require('plugins/customTileMap/ui/index_patterns/_intervals'));
    let getComputedFields = require('plugins/customTileMap/ui/index_patterns/_get_computed_fields');
    let DocSource = Private(require('plugins/customTileMap/ui/courier/data_source/doc_source'));
    let mappingSetup = Private(require('plugins/customTileMap/ui/utils/mapping_setup'));
    let FieldList = Private(require('plugins/customTileMap/ui/index_patterns/_field_list'));

    let flattenHit = Private(require('plugins/customTileMap/ui/index_patterns/_flatten_hit'));
    let formatHit = require('plugins/customTileMap/ui/index_patterns/_format_hit');
    let calculateIndices = Private(require('plugins/customTileMap/ui/index_patterns/_calculate_indices'));

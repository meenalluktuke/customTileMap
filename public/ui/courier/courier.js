define(function (require) {
  ...
  require('plugins/customTileMap/ui/es');
  require('plugins/customTileMap/ui/promises');
  require('plugins/customTileMap/ui/safe_confirm');
  require('plugins/customTileMap/ui/index_patterns');

  require('plugins/customTileMap/ui/modules').get('kibana/courier')
  .service('courier', function ($rootScope, Private, Promise, indexPatterns, Notifier) {
    function Courier() {
      let self = this;

      let DocSource = Private(require('plugins/customTileMap/ui/courier/data_source/doc_source'));
      let SearchSource = Private(require('plugins/customTileMap/ui/courier/data_source/search_source'));
      let searchStrategy = Private(require('plugins/customTileMap/ui/courier/fetch/strategy/search'));

      let requestQueue = Private(require('plugins/customTileMap/ui/courier/_request_queue'));
      let errorHandlers = Private(require('plugins/customTileMap/ui/courier/_error_handlers'));

      let fetch = Private(require('plugins/customTileMap/ui/courier/fetch/fetch'));
      let docLooper = self.docLooper = Private(require('plugins/customTileMap/ui/courier/looper/doc'));
      let searchLooper = self.searchLooper = Private(require('plugins/customTileMap/ui/courier/looper/search'));

      // expose some internal modules
      self.setRootSearchSource = Private(require('plugins/customTileMap/ui/courier/data_source/_root_search_source')).set;

      self.SavedObject = Private(require('plugins/customTileMap/ui/courier/saved_object/saved_object'));
      self.indexPatterns = indexPatterns;
      self.redirectWhenMissing = Private(require('plugins/customTileMap/ui/courier/_redirect_when_missing'));

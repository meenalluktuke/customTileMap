define(function (require) {
  return function FetchTheseProvider(Private, Promise) {
    let notify = Private(require('plugins/customTileMap/ui/courier/fetch/_notifier'));
    let forEachStrategy = Private(require('plugins/customTileMap/ui/courier/fetch/_for_each_strategy'));

    // core tasks
    let callClient = Private(require('plugins/customTileMap/ui/courier/fetch/_call_client'));
    let callResponseHandlers = Private(require('plugins/customTileMap/ui/courier/fetch/_call_response_handlers'));
    let continueIncomplete = Private(require('plugins/customTileMap/ui/courier/fetch/_continue_incomplete'));

    let ABORTED = Private(require('plugins/customTileMap/ui/courier/fetch/_req_status')).ABORTED;
    let DUPLICATE = Private(require('plugins/customTileMap/ui/courier/fetch/_req_status')).DUPLICATE;
    let INCOMPLETE = Private(require('plugins/customTileMap/ui/courier/fetch/_req_status')).INCOMPLETE;
